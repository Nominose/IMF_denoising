...
def _detect_base():
    # /gpfs/work/aac/xingyiyao23 is the HPC cluster root; /host/d* are the docker mounts.
...
def _he_bins(name):
    """Histogram-equalization bins live under Data/ on the HPC cluster, file/ on the docker box."""
    for sub in ('Data/histogram_equalization', 'file/histogram_equalization'):
        c = os.path.join(_BASE, sub, name)
        if os.path.isfile(c):
            return c
    return os.path.join(_BASE, 'file/histogram_equalization', name)
...
def main():
    ...
    bins = np.load(_he_bins('bins.npy')) if HE else None
    binsm = np.load(_he_bins('bins_mapped.npy')) if HE else None
