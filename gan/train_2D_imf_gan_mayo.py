...
HPC/SLURM (cluster): sbatch gan/run_train_imf_gan_mayo.sh   (paths auto-resolve to /gpfs via _detect_base()).
...
def _detect_base():
    """Data root holding the Mayo recon tree + projects/denoising. HPC cluster root first (data at
    <base>/新mayo_data/), then the docker D: mounts (data at <base>/Data/新mayo_data/). Mirrors
    CT_experiments/train_2D_imf_mayo.py so the GAN run resolves the same paths as the flow pretrain."""
    for b in ('/gpfs/work/aac/xingyiyao23', '/host/d/research', '/host/d', 'D:/research', '/d/research'):
        if (os.path.isdir(os.path.join(b, '新mayo_data'))
                or os.path.isdir(os.path.join(b, 'Data', '新mayo_data'))
                or os.path.isdir(os.path.join(b, 'projects/denoising'))):
            return b
...
def _mayo_data_root(base):
    """The Mayo data dir under `base`: <base>/新mayo_data on the HPC cluster, <base>/Data/新mayo_data
    in docker. Return whichever exists (HPC layout first)."""
    hpc = os.path.join(base, '新mayo_data')
    dock = os.path.join(base, 'Data', '新mayo_data')
    return hpc if os.path.isdir(hpc) else (dock if os.path.isdir(dock) else hpc)


_MAYO_DATA = _mayo_data_root(_BASE)


def _remap(p):
    """Resolve a stored recon path onto the current data root. The xlsx records the recon files as
    `/host/e/D/Data/low_dose_CT/<rest>` (old E: layout). On HPC the whole tree is uploaded under
    <base>/新mayo_data/<rest>; in docker it lives on D: at <base>/Data/新mayo_data/<rest>. Try the
    stored path as-is first (works if E: is mounted), then remap onto _MAYO_DATA (no E: mount needed)."""
    if p is None:
        return p
    p = str(p)
    if os.path.exists(p):
        return p
    # generic: everything after '/low_dose_CT/' lives directly under _MAYO_DATA (HPC + docker)
    if '/low_dose_CT/' in p:
        cand = os.path.join(_MAYO_DATA, p.split('/low_dose_CT/', 1)[1])
        if os.path.exists(cand):
            return cand
    # legacy exact-prefix fallback (old docker E: layout)
