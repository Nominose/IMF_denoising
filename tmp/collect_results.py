...
def _detect_models_root():
    """The cluster root first, then the docker D: mounts, then the original Windows path."""
    for b in ("/gpfs/work/aac/xingyiyao23", "/host/d/research", "/host/d"):
        c = os.path.join(b, "projects/denoising/models")
        if os.path.isdir(c):
            return c
    return r"D:\research\projects\denoising\models"


DEFAULT_MODELS = os.environ.get("MODELS_ROOT") or _detect_models_root()
