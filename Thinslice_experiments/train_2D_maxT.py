"""
train_2D_maxT.py — Ablation: cDDPM trained with only t=T (max noise level)

Purpose: Verify whether multi-scale training is the key mechanism behind
cDDPM's advantage over direct N2N in few-step DDIM inference.
If t=T-only training degrades few-step performance, it supports
the multi-scale representation shaping hypothesis.
"""
import sys
...
# ========== Parameters ==========
trial_name = 'cddpm_maxT_unsupervised_gaussian_brainCT'
...
diffusion_model = ddpm.GaussianDiffusion(
    ...
    train_only_max_t=True,  # KEY: only train at t=T (max noise level)
)
