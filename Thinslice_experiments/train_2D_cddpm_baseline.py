"""
train_2D_cddpm_baseline.py — Baseline: cDDPM trained over the full t range (no restriction)

Purpose: Reference baseline for the t-range ablation series (maxT / highT / lowT).
Identical to train_2D_maxT.py in every config except that train_only_max_t is
left at its default (False), so the model sees the full timestep range.

Naming convention follows the ablation series:
  cddpm_baseline_unsupervised_gaussian_brainCT  (full t range)
  cddpm_maxT_unsupervised_gaussian_brainCT      (only t = T)
  cddpm_highT_unsupervised_gaussian_brainCT     (only t in [800, 999])
  cddpm_lowT_unsupervised_gaussian_brainCT      (only t in [0, 199])
"""
import sys
...
# ========== Parameters ==========
trial_name = 'cddpm_baseline_unsupervised_gaussian_brainCT'
...
diffusion_model = ddpm.GaussianDiffusion(
    ...
    # NOTE: no train_only_max_t / train_t_range here — full t range training.
)
