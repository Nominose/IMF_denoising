...
def _detect_base():
    ...
    docker D: mounts. The data is uploaded as <base>/新mayo_data/ (xlsx + simulation_highnoise_v2/)."""
...
def _remap(p):
    """Resolve a stored recon path onto the current base. The xlsx stores E:-drive docker paths like
    /host/e/D/Data/low_dose_CT/...; on the HPC cluster that whole tree is uploaded under
    <base>/新mayo_data/ (so .../low_dose_CT/simulation_highnoise_v2/X -> <base>/新mayo_data/simulation_highnoise_v2/X).
    Docker fallbacks (E: / D: mounts) are kept so the script still runs in the original environment."""
    if p is None:
        return p
    p = str(p)
    if os.path.exists(p):
        return p
    # HPC: the entire low_dose_CT data tree lives under <base>/新mayo_data/
    if '/low_dose_CT/' in p:
        cand = os.path.join(_BASE, '新mayo_data', p.split('/low_dose_CT/', 1)[1])
        if os.path.exists(cand):
            return cand
    # docker fallbacks (E: / D: drives)
